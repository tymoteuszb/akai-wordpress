...
function srz_fb_render_fullpage($album,$images,$common_options){
	...
	for($j=$jf_start;$j<$jf_end;$j++){
		...
		$link = '<a class="aimg" style="width:'.$album['thumbwidth'].'px; height:'.$album['thumbheight'].'px;" href="'. $image['link_fb'].'" title="'.  nl2br($image['txt']).'">'; //'.$common_options['lightboxattrib'].' wywalone z końca tagu
...
function srz_fb_get_album_api($albumids, $shuffle_images, $cachetime){
	...
	foreach($albumids_arr as $albumid){
		...
		if(!$contents or isset($_GET['forcesync'])){
			$url = 'http://graph.facebook.com/'.$albumid.'/photos?fields=link,picture';
